...
public class Commander implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String arg, String[] args) {
        if ((args.length == 1) || (args.length == 2) || (args.length == 3)) {
            ...
            if (args[0].equalsIgnoreCase("give")) {
                ...
                if (item == null) {
                    sender.sendMessage(
                            ChatColor.RED + "GuiShopManager Item " + ChatColor.GOLD + itemname + ChatColor.RED
                                    + " not found!");
                    return false;
                }
                ...
                if (player == null) {
                    sender.sendMessage(
                            ChatColor.RED + "Player " + ChatColor.GOLD + playername + ChatColor.RED + " not found!");
                    return false;
                }

                item.giveItem(player, GSMItems.GSMGiveItemsReason.COMMAND);
                sender.sendMessage(
                        ChatColor.YELLOW + "Added GSM Item " + ChatColor.RED + itemname + ChatColor.YELLOW + " to "
                                + ChatColor.RED + playername + "'s" + ChatColor.YELLOW + " Inventory.");
    ...
    private void sendInfo(CommandSender s) {
        ...
        s.sendMessage(ChatColor.YELLOW + "/GSM give <itemname> [Player] " + ChatColor.RED
                + "- Gives [Player] the named item");
    }
